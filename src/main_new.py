...
def test_program(program_code: str, task: Dict) -> Tuple[float, List[Tuple[Any, Any, bool]]]:
    ...
    try:
        ...
        for example in task['test']: #TODO: change back to 'test' for final eval
            inp = example['input']
...
def phase1_find_similar(
    ...
    """
    Phase 1: Randomly sample programs from library.

    Args:
        task: Task dictionary with 'train' examples
        task_id: Unique task identifier
        library: Program library
        timeout: Timeout per program execution (unused in random sampling)
        num_samples: Number of random programs to sample

    Returns:
        Phase1Result with randomly sampled programs
    """
    try:
        # Get all programs from library
        all_programs = library.get_all_programs()
        ...
        sample_size = min(num_samples, len(all_programs))
        sampled_programs = random.sample(all_programs, sample_size)

        # Format sampled programs to match expected structure
        similar_programs = [
            {
                'task_id': prog['task_id'],
                'program': prog['program'],
                'similarity': 0.0  # No similarity computed for random sampling
            }
            for prog in sampled_programs
        ]

        # Test sampled programs for perfect match
        best_library_score = 0.0
        best_library_program = None
        perfect_match = False

        for prog in similar_programs:
            score, _ = test_program(prog['program'], task)
            if score > best_library_score:
                best_library_score = score
                best_library_program = prog['program']
                if score == 1.0:
                    perfect_match = True
                    break
