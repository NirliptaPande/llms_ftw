...
def is_symmetric(grid: Grid) -> Boolean:
    """Checks if the grid is symmetric (horizontal, vertical, or diagonal)"""
    return (grid == horizontal_mirror(grid) or
            grid == vertical_mirror(grid) or
            grid == diagonal_mirror(grid) or
            grid == counterdiagonal_mirror(grid) or
            grid == rot180(grid))


def has_enclosed_area(grid: Grid) -> Boolean:
    """Checks if there are enclosed background areas"""
    bg = most_common_color(grid)
    # Get all background objects
    bg_objs = as_objects(grid, True, False, False) # univalued=True, diagonal=False, without_bg=False
    bg_objs = color_filter(bg_objs, bg)
...
def is_monochromatic(obj: Object) -> Boolean:
    """Checks if an object has only one color"""
    return len(set(c for c, _ in obj)) == 1